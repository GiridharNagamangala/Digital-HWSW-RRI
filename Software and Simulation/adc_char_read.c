...
int bramToFile (volatile uint32_t* bram_ptr, int reset, int done);

int main(int argc, char **argv) {

    if (argc < 2) { //look for 2 arguments 
    printf("Usage: %s <trigger: 1 for run, 0 for reset>\n", argv[0]);
    return 1;
  }
    ...
    while (*gpio_in != 1);  // while loop to run until gpio_in receives done high
    // sleep(1000000);

        printf("Done detected, Reading samples...\n");

    bramToFile(bram_ptr, trigger, *gpio_in);
...
int bramToFile (volatile uint32_t* bram_ptr, int trigger, int done) {

  if ((trigger == 0) || (done == 0)) return 1;
  else {
